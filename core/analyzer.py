...
class StateAnalyzer:
    ...
    def win_probability(self, intel: Dict, target: Dict) -> float:
        ...
        heal_hp_avail  = heal_stats["heal_hp_total"]
        heal_ep_avail  = heal_stats["heal_ep_total"]
        ...
        sim_heals    = list(heal_stats["items"][:])
        sim_wins     = False
        HEAL_THRESHOLD = 35.0
        ...
        heal_buffer = min(0.10, heal_hp_avail / 200.0)
        prob += heal_buffer
    ...
    # -------------------------------------------------------------------------
    # [FIX] Methods below were accidentally defined OUTSIDE the class
    # (missing indentation). Now correctly inside StateAnalyzer.
    # -------------------------------------------------------------------------

    def calc_turns_to_kill(self, attacker: Dict, defender: Dict) -> int:
        """Estimate number of attack actions needed to kill defender."""
        a_bonus, _ = self.get_equipped_bonus(
            attacker.get("equipped_weapon") or attacker.get("equippedWeapon")
        )
    ...
    def detect_game_phase(self, intel: Dict) -> str:
        """
        Classify current game phase as 'early', 'mid', or 'late'.
        Based on game_day and alive_count from intel.
        """
    ...
    def zone_risk_score(self, intel: Dict) -> float:
        """Score danger level of current region based on DZ status."""
        score = 0.0
    ...
    def estimate_win_probability(self, intel: Dict, target: Dict,
                                  enemy_profile: Optional[Dict] = None) -> float:
        """
        Full win probability estimate with phase, weapon, zone, HP/EP adjustments.
        This is the main method strategy.py should call for PvP decisions.
        """
        ...
        if phase == "early":
            ...
            score += 0.03
        score += max(-0.10, min(0.10,
            (intel.get("hp", 100) - target.get("hp", 100)) * 0.002))
        score += max(-0.08, min(0.08,
            (intel.get("ep", 0) - target.get("ep", 0)) * 0.01))
        my_ttk   = self.calc_turns_to_kill(
            {**intel, "equippedWeapon": intel.get("equipped_weapon")}, target)
        their_ttk = self.calc_turns_to_kill(
            {**target, "equipped_weapon": target.get("equippedWeapon")}, intel)
    ...
    def score_monster_target(self, intel: Dict, monster: Dict) -> float:
        """Score a monster as a hunting target (higher = more worth fighting)."""
        ...
        turn_cost   = self.calc_turns_to_kill(
            {**intel, "equippedWeapon": intel.get("equipped_weapon")},
            {"hp": hp, "def": {"wolf": 1, "bear": 2, "bandit": 3}.get(mtype, 2)}
        ) * 0.03
        return win_prob + reward_bias - risk_penalty - turn_cost

    def facility_value_score(self, intel: Dict, facility: Dict) -> float:
        """Score how valuable it is to use a given facility right now."""
    ...
    def get_useful_facility(self, intel: Dict) -> Optional[Dict]:
        ...
        if not facilities:
            return None
        scored = sorted(
            ((self.facility_value_score(intel, f), f) for f in facilities),
            key=lambda x: x[0],
            reverse=True
        )
